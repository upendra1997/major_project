...
class Instructor:
    def __init__(self, opt):
        ...
        if 'bert' in opt.model_name:
            ...
            fnames = [opt.dataset_files[x]['train'] for x in opt.dataset_files]
            fnames = fnames + [opt.dataset_files[x]['test'] for x in opt.dataset_files]
            tokenizer = build_tokenizer(
                fnames= fnames,
                max_seq_len=opt.max_seq_len,
                dat_fname='{0}_tokenizer.dat'.format("".join(opt.datasets)))
            embedding_matrix = build_embedding_matrix(
                word2idx=tokenizer.word2idx,
                embed_dim=opt.embed_dim,
                dat_fname='{0}_{1}_embedding_matrix.dat'.format(str(opt.embed_dim), "".join(opt.datasets)))
            self.model = opt.model_class(embedding_matrix, opt).to(opt.device)

        trainset = ConcatDataset([ABSADataset(opt.dataset_files[x]['train'], tokenizer) for x in opt.dataset_files])
        # trainset = ABSADataset(opt.dataset_file['train'], tokenizer)
        testset = ConcatDataset([ABSADataset(opt.dataset_files[x]['test'], tokenizer) for x in opt.dataset_files])
    ...
    def run(self):
        ...
        self.optimizer = self.opt.optimizer(self._params, lr=self.opt.learning_rate, weight_decay=self.opt.l2reg)

        self._reset_params()
        max_test_acc, max_f1 = self._train(self.criterion, self.optimizer)
